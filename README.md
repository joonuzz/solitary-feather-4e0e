...
> [!IMPORTANT]
> When using C3 to create this project, select "no" when it asks if you want to deploy. You need to follow this project's [setup steps](https://github.com/cloudflare/templates/tree/main/solitary-feather-4e0e#setup-steps) before deploying.
...
```bash
npm create cloudflare@latest -- --template=cloudflare/templates/solitary-feather-4e0e
```

A live public deployment of this template is available at [https://solitary-feather-4e0e.templates.workers.dev](https://solitary-feather-4e0e.templates.workers.dev)
...
1. Install the project dependencies with a package manager of your choice:
   ...
   npm install
   ```
2. Create a [D1 database](https://developers.cloudflare.com/d1/get-started/) with the name "plain-flower-b984-d1":
   ```bash
   npx wrangler d1 create plain-flower-b984-d1
   ```
   ...and update the `database_id` field in `wrangler.json` with the new database ID.
3. Run the following db migration to initialize the database (notice the `migrations` directory in this project):
   ```bash
   npx wrangler d1 migrations apply --remote plain-flower-b984-d1
   ```
